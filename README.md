...
# 20230410讨论记录
工作总结：
技术上，这周主要完成后端各部分的拼接，对后端各模块进行测试并优化位置度的计算。
近期还需要将前端和后端联合，完成整个软件的搭建和测试。未来尝试自主学习神经网络，在人工标注的辅助下将新数据集加入训练集中，逐步迭代识别模型以提高精度

技术细节：
- 通过planned_location和actual_location中匹配点之间的重投影误差来求解Homography矩阵，然后将计划坐标系中的点转换为图像坐标系中的点
- 先将整张图片缩小8倍，进行大模板的匹配，以此减少计算量。然后再在原图大小上截取面板
